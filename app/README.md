...
### `users/{uid}`
Written by `FirestoreService.saveUserData` (sign-up) and
`updateUserProfile` (profile edits).
...
| `fullName` | string | |
| `phoneNumber` | string | |
| `savedAddress.street` | string | Nested map |
| `savedAddress.brgy` | string | Nested map |
| `emergencyContactName` | string | |
| `emergencyContactNumber` | string | |
| `createdAt` | timestamp | Server timestamp, set once on sign-up |
| `updatedAt` | timestamp | Server timestamp, refreshed on every save |

### `reports/{reportId}`
Written by `distress_report.dart` (emergency) and `concern_report.dart`
(community); read by the Emergency, Community, Profile, and Notifications
screens.

| Field | Type | Notes |
|---|---|---|
| `reportType` | string | `'emergency'` or `'community'` |
| `category` | string | Selected category label |
| `barangay` | string | |
| `locationType` | string | `'GPS'`, `'PIN'`, or `'LANDMARK'` |
| `landmarkDetails` | string? | Only set when `locationType == 'LANDMARK'` |
| `coordinates` | GeoPoint? | Only set for `GPS`/`PIN` modes |
| `natureOfDistress` / `natureOfConcern` | string | Free-text description (field name depends on report type) |
| `userId` | string | Reporter's UID (`'anonymous'` allowed for emergency reports from signed-out users) |
| `userName`, `userPhone` | string | Snapshot of the reporter's contact info at submission time |
| `status` | string | `'pending'` → ... → `'resolved'` / `'closed'` / `'withdrawn'` |
...
| Color | Hex | Used for |
|---|---|---|
| Brand terracotta | `#C8532B` | Primary brand accent, bottom nav background |
| Emergency gradient | `#F9C7B0` | Emergency tab background gradient |
| Community gradient | `#FBEBC9` | Community tab background gradient |
| Profile gradient | `#F7D8B8` | Profile tab background gradient |
| Healthcare gradient | `#F6D3D3` | Healthcare tab background gradient |
| Services gradient | `#F3E4CF` | Services tab background gradient |
| Text primary | `#2B1D19` | Headings/body text |
| Text secondary | `#8C7B73` | Labels, captions |

Cards use consistent rounded corners (`BorderRadius.circular(10–16)`), soft
drop shadows, and section headers styled as a small bold caps label with a
trailing divider line.

## 8. Setup

1. Install Flutter and run `flutter pub get`.
2. Configure Firebase for the project (Firestore + Phone Auth enabled) and
   regenerate `lib/firebase_options.dart` with the FlutterFire CLI if setting
   up a new Firebase project:
   ```
   flutterfire configure
   ```
3. Ensure `assets/svg/logo.svg` exists and is registered in `pubspec.yaml`
   (referenced by `main.dart`).
4. Run the app:
   ```
   flutter run
   ```
...
- Several Services-tab entries (Legal, Permits) are placeholders that show a
  "Coming soon" message — no backend/URL wired up yet.
- `lib/_unused_legacy/` contains superseded page/widget versions kept for
  reference only; delete once confirmed unnecessary.
